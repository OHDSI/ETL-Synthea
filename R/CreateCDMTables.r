...
#' @param cdmSchema  The name of the CDM database schema that will contain the Vocabulary
#'                                     instance.  Requires read and write permissions to this database. On SQL
#'                                     Server, this should specifiy both the database and the schema,
#'                                     so for example 'cdm_instance.dbo'.
#' @param cdmVersion Your CDM version.  Currently "5.3" and "6.0" are supported.
#'
...
CreateCDMTables <- function (connectionDetails,cdmSchema,cdmVersion)
{
	...
	supportedVersions <- c("5.3","6.0")
	rdbms <- tolower(connectionDetails$dbms)
	
	if (!(cdmVersion %in% supportedVersions)) {
		stop("Unsupported CDM specified. Supported CDM versions are \"5.3\" and \"6.0\"")
	}
