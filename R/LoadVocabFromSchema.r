...
LoadVocabFromSchema <- function (connectionDetails, vocabSourceSchema, vocabTargetSchema)
{
  ...
  for (tableName in vocabTableList) {
	  sql <- paste0("create table ",vocabTargetSchema,".",tableName," with (distribution=replicate, clustered columnstore index) as select * from ",vocabSourceSchema,".",tableName)
