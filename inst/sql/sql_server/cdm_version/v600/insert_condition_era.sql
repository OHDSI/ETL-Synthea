...
(
	SELECT
		...
		, COALESCE(NULLIF(co.condition_end_datetime,NULL), dateadd(day,1,condition_start_datetime)) AS condition_end_datetime
	...
	(
		...
		(
			...
			SELECT
				...
				, condition_concept_id ORDER BY condition_start_datetime) AS start_ordinal
...
JOIN cteEndDates e ON c.person_id = e.person_id AND c.condition_concept_id = e.condition_concept_id AND e.end_date >= c.condition_start_datetime
GROUP BY
